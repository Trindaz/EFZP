# Modified by Michael Schmid on 2024-01-16 to work with Python 3
# and add additional salutations and signatures.
...
def is_word_likely_in_signature(word, text_before="", text_after=""):
    ...
    # is it capitalized?
    if word[:1] in string.ascii_uppercase and word[1:2] in string.ascii_lowercase:
        return True
...
def get_body(
    email_text, check_salutation=True, check_signature=True, check_reply_text=True
):
...
def get_salutation(email_text):
    ...
    pattern = (
        "\s*(?P<salutation>("
        + "|".join(salutation_opening_statements)
